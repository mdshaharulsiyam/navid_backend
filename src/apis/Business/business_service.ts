...
const update = async (id: string, data: IBusiness) => {
    ...
    const result = await business_model.findOne({ _id: id, user: data.user }, {
        $set: {
            ...data
        }
    }, { new: true })
...
const block_shop = async (id: string, user: string) => {
    ...
    const result = await business_model.findOneAndUpdate({ _id: id, user }, {
        $set: {
            $cond: {
                if: { $eq: ["$block", false] },
                then: true,
...
const get_all = async (queryKeys: QueryKeys, searchKeys: SearchKeys, populatePath?: any, selectFields?: string | string[]) => {
    return await Queries(business_model, queryKeys, searchKeys, populatePath, selectFields);
}

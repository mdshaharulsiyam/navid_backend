...
const create = async (req: Request, res: Response) => {
    ...
    const business_documents = !Array.isArray(req.files) && req.files?.business_documents && req.files.business_documents.length > 0 && req.files.business_documents.map((doc: any) => doc.path) || [];
...
const get_all = async (req: Request, res: Response) => {
    ...
    const selectFields = '';
    const result = await business_service.get_all(queryKeys, searchKeys, populatePath, selectFields)
...
const approve_shop = async (req: Request, res: Response) => {
    ...
    const result = await business_service.approve_shop(req?.params?.id, req?.user?._id as string)
...
const block_shop = async (req: Request, res: Response) => {
    ...
    const result = await business_service.block_shop(req?.params?.id, req?.user?._id as string)

...
async function refund(payment_intent: string, user_password: string, inserted_password: string, amount?: string | number) {
    ...
    if (refund?.id) {
        ...
        await payment_model.updateOne({ transaction_id: payment_intent }, { $set: { refund: refund?.id } })
        return {
            success: true,
...
async function transfer_balance(account_id: string, amount: number, currency?: string) {
...
async function check_payment_status(id: string) {
    ...
    const payment_intent_retrieve = await stripe.paymentIntents.retrieve(payment_intent as string);

    if (!payment_intent_retrieve || payment_intent_retrieve.amount_received === 0) {
...
async function validate_transfer_balance(user: IAuth, data: { [key: string]: string | number }) {

    const is_match_pass = await bcrypt.compare(data?.password as string, user?.password);
